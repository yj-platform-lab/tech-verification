...
- EC2 コンソール「接続」に表示される各方式の位置づけ

## 背景

VMware 環境では、リモートコンソールを用いて OS の起動状況確認や障害対応を行うことが一般的である。同じ感覚で EC2 にも「画面に直接つながる接続」があるのかを確認した。

調べた結果、EC2 の接続方式は通常運用向け（SSH / Session Manager） と障害対応向け（シリアルコンソール） に明確に分かれており、VMware とは思想が異なることが分かった。

本記事では、この違いを整理し「どれを選ぶべきか」を明確にする。
...
SSH を使う点は同じだが、ローカルに鍵ファイルを置かずに接続できる方式。仕組みとしては「①SSH公開鍵をInstance Connect APIでインスタンスメタデータにプッシュ」、「②インスタンスメタデータから取得した公開鍵でユーザを認証する」というもの。以下の特徴がある。
...
では実際に試してみよう。インバウンドのルールを削除していま下記のような状態となっている
...
接続すると以下のような画面となる。接続ユーザがssm-userであることがポイント。
